...
// const colorArrayLength = 400; // Assuming this is still relevant for your code's logic
...
//     if (!isNaN(colorIndex) && colorIndex >= 0 && colorIndex < colorArrayLength) {
//       currentColorIndex = colorIndex; // Directly update the global currentColorIndex variable
//       console.log("Manual Color Index Set To: ", colorIndex);
//       // Here, you can call the function to apply the selected effect based on the new index
//     } else {
//       alert("Please enter a valid number within the range of available colors.");
//     }
//   });
// });

// function randomEffectChanger(speed, numberOfGroups) {
//     const effects = organizeEffects();
//     const effectGroups = Object.values(effects);
//     const allEffects = effectGroups.flat(); // Flatten all effect groups into a single array

//     const applyEffect = () => {
//         if (allEffects.length === 0) {
//             console.error('No effects available to apply.');
//             return;
//         }
        
//         // Use currentColorIndex to select an effect, ensuring it's within the bounds of allEffects array
//         const selectedEffectIndex = currentColorIndex % allEffects.length;
//         const selectedEffect = allEffects[selectedEffectIndex];
        
//         console.log('Selected Effect:', selectedEffect);
//         // Implement the logic to apply the selected effect here
//     };

//     // Apply the effect immediately and then set an interval if needed
//     applyEffect();
//     // If you want to continuously cycle through effects, uncomment the following line:
//     // setInterval(applyEffect, speed);


// // Make sure to call `randomEffectChanger` with appropriate parameters where needed


//     // Set an interval to change effects at the specified speed
//     // setInterval(selectRandomEffect, speed);


// // Example usage:
// // Change effect every 2000 milliseconds (2 seconds), cycling through 3 different groups of effects
// // randomEffectChanger(2000, 3);
// }


// function organizeEffects(angle, tm, v) {
//     console.log('Entering organizeEffects');
//     if (!v || !v.length || v[0].z === undefined) {
//         console.error('Variable v is undefined or does not have the expected structure');
//         console.log('Value of v:', v);
//         return 'rgb(255, 255, 255)'; // Return a default color if `v` is not structured properly
//     }

//     // Combine all arrays into a larger array
//     const allEffects = [
//         theCrazyOne,
//         multiColourFlashes,
//         googlyEyes,
//         singleDualColourDisco,
//         discoBalls,
//         colourFadeEffects
//     ];

    // // Calculate which array to use based on `angle`
    // const selectedArrayIndex = angle % allEffects.length;
    // const selectedEffectArray = allEffects[selectedArrayIndex];

    // // Calculate which line to use within the selected array based on `tm`
    // const selectedEffectIndex = tm % selectedEffectArray.length;
    // const selectedEffectLine = selectedEffectArray[selectedEffectIndex];

    // Use `currentColorIndex` to select an effect directly from `allEffects`
    // Ensuring it's within the bounds of `allEffects` array
    // const selectedEffectIndex = currentColorIndex % allEffects.length;
    // const selectedEffectLine = allEffects[selectedEffectIndex];

    // color effect arrays - multiple different length arrays of colour settings
  // Function to get colors based on angle, tm, and v for the given cci
  function getColorsByCCI(angle, tm, v, cci) {
    // Check if cci is within bounds of colorEffectArrays
    if (cci >= 0 && cci < colorEffectArrays.length) {
      // Access the color effect array using cci as index
      const colorEffectArray = colorEffectArrays[cci];
      // Call the appropriate color effect function with the provided parameters
      return colorEffectArray(angle, tm, v);
    } else {
      // If cci is out of bounds, return a default color or handle the error as needed
      return ['default-color']; // You can change this to return a default color or handle the error differently
    }
  }

  // Example usage:
  const colors = getColorsByCCI(angle, tm, v, cci);
    ...
    function getColors2(angle, tm, v) {
      return [
...
//     // The Crazy One - Array of 1
//     const theCrazyOne = [
//         `Math.random() * Math.floor((v[0].z + R) / (2 * R) * 255) > 128 ? \`rgb(${
//             Math.random() * Math.floor((v[0].z + R) / (2 * R) * 255)
//         }, ${Math.random() * Math.floor((v[0].z + R) / (2 * R) * 255)}, ${
//             Math.random() * Math.floor((v[0].z + R) / (2 * R) * 255)
//         })\` : 'alternative-color'` //  #0 - The Crazy One
//     ];

//     // MultiColour Flashes - Array
//     const multiColourFlashes = [
//         `rgb(${Math.floor(Math.sin(Date.now() / 550) * 127 + 128)}, ${Math.floor(Math.sin(Date.now() / 575) * 127 + 128)}, ${Math.floor(Math.sin(Date.now() / 6000) * 127 + 64)})`, // #108 Muted gold to orange flicker
//         `rgb(${Math.floor(Math.sin(Date.now() / 620) * 127 + 64)}, ${Math.floor(Math.sin(Date.now() / 640) * 127 + 255)}, ${Math.floor(Math.sin(Date.now() / 660) * 127 + 128)})`, // #109 Neon green and violet flash
//         `rgb(${Math.floor(Math.sin(Date.now() / 780) * 127 + 64)}, ${Math.floor(Math.sin(Date.now() / 800) * 127 + 255)}, ${Math.floor(Math.sin(Date.now() / 820) * 127 + 64)})`, // #112 Cool mint flash
//         // #95 Quick flashing red to orange
...
//   return eval(selectedEffectLine); // Evaluate the string to execute any embedded JavaScript expressions
