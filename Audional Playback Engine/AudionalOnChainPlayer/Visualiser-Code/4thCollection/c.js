...
let initialTm = Date.now(); // Could be the initial timestamp or another metric
let initialV = [{x: 0, y: 0, z: 0}]; // Adjust z as needed or remove if unnecessary



function logColorSelection(index, colorData) {
  if (!loggedSelections.has(index)) {
      console.log(`Selection Index: ${index}, Color Data: ${colorData}`);
      loggedSelections.add(index);
  }
}
...
const colorLogger = {
  ...
  // Record and log a single selection
  record(index, colorData) {
    if (!this.loggedIndices.has(index)) {
      this.loggedIndices.add(index);
      // Only log the selected color data, not the entire array
      console.log(`Unique Color Selection: Index - ${index}, Data - ${colorData}`);
      this.history.push({ index, colorData }); // Optionally, you can still push the entire array to history if needed
    }
  },

  // Within colorLogger definition
  logInitialSelection(initialIndex, colors) {
    if (!this.loggedIndices.has(initialIndex)) {
      const initialColorData = colors[initialIndex];
      console.log(`Initial CCI2 Selection: Index - ${initialIndex}, Data - ${initialColorData}`);
      this.record(initialIndex, initialColorData); // This ensures it's added to the history and logged indices
    }
  },
  ...
  clearHistory() {
    this.history = [];
    this.loggedIndices.clear();
  }
};

function selectAndLogColor(angle, tm, v) {
  const colors = getColors(angle, tm, v);
  // Implement your selection logic here; example uses random selection
  const selectionIndex = Math.floor(Math.random() * colors.length);
  const selectedColor = colors[selectionIndex];

  // Record and log the selection if it's unique
  colorLogger.record(selectionIndex, selectedColor);
  
}

// Example function that uses initialCCI2 for the first color selection
function logInitialCCI2ColorSelection(angle, tm, v) {
  const colors = getColors(angle, tm, v, true); // Assuming this call is correct
  const initialIndex = calculateCCI2(initialCCI2); // This needs to map to an index properly
  const selectedColor = colors[initialIndex % colors.length];
  colorLogger.record(initialIndex, selectedColor);
}

// Call this function with properly defined initial values
logInitialCCI2ColorSelection(initialAngle, initialTm, initialV);
