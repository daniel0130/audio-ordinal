...
function logSelection(index, color) {
    console.log(`Selected Index: ${index}, Color: ${color}`);
}

// Function to simulate seeded random generation
function seededRandom(seedValue) {
    const x = Math.sin(seedValue) * 10000;
    return x - Math.floor(x);
}

// Function to calculate dynamic index based on a seed
function calculateDynamicIndex(channelIndex) {
    const randomSeed = seed + channelIndex;
    const randomMultiplier = seededRandom(randomSeed) * 141;
    return Math.floor(randomMultiplier) + 1;
}

// Handler for applying effects based on the dynamic index
function applyDynamicEffect() {
    const effectIndex = calculateDynamicIndex(currentEffectIndex); // Updated to use calculateDynamicIndex
    console.log(`Effect Index: ${effectIndex}, Current Index: ${currentEffectIndex}`);
    getColors(effectIndex, performance.now(), /* vertex data */); // Example call, replace with actual vertex data
}


    // Select a color based on some logic
    const selectedColorIndex = Math.floor(Math.random() * colors.length); // Example logic for selecting a color index

    logSelection(selectedColorIndex, colors[selectedColorIndex]);
    return colors[selectedColorIndex]; // Adjusted to return the selected color
}

// New function to handle start/stop actions and effect application
function initiateEffectApplication(data) {
    if (data.action === 'stop') {
        currentEffectIndex = initialEffectIndex;
        console.log('Animation stopped, index reset to initial value.');
    } else if (data.action === 'start') {
        const { channelIndex } = data;
        currentEffectIndex = calculateDynamicIndex(channelIndex);
        console.log(`Index updated to ${currentEffectIndex}, applying corresponding effect.`);
        applyDynamicEffect();
    }
}

// Example of an event listener for external controls (e.g., via BroadcastChannel)
const playbackControlChannel = new BroadcastChannel('playback_control');
playbackControlChannel.onmessage = initiateEffectApplication;
