...
// New structure to hold the pre-calculated schedule
const sequenceSchedule = [];

// Function to pre-calculate the entire sequence schedule
function calculateSequenceSchedule() {
    const totalSequences = window.unifiedSequencerSettings.getTotalSequences();
    for (let sequenceIndex = 0; sequenceIndex < totalSequences; sequenceIndex++) {
        const currentSequence = window.unifiedSequencerSettings.getSequenceSettings(sequenceIndex);
        const scheduleForSequence = [];
        for (let stepIndex = 0; stepIndex < 4096; stepIndex++) {
            const stepData = { channels: [] };
            for (let channelIndex = 0; channelIndex < 16; channelIndex++) {
                const channelSettings = currentSequence[`ch${channelIndex}`];
                if (channelSettings && channelSettings.steps[stepIndex] && !channelSettings.mute) {
                    stepData.channels.push({ channelIndex, play: true });
                }
            }
            scheduleForSequence.push(stepData);
        }
        sequenceSchedule.push(scheduleForSequence);
    }
}

// Function to start the sequencer and pre-calculate the schedule
function startSequencer() {
    calculateSequenceSchedule();
    startScheduler();
}

let currentSequenceIndex = 0; // Assuming a global variable to track the current sequence index
let currentStepIndex = 0; // Track the current step index within the sequence

// Optimized scheduler function
function scheduleNextStep() {
    if (isPaused) {
        console.log("Sequencer is paused/stopped. Not scheduling the next step.");
        return; // Exit the function if the sequencer is paused or stopped
    }    const currentBPM = window.unifiedSequencerSettings.getBPM();
    const stepDuration = 60 / currentBPM / 4; // Assuming 4 steps per beat
    const nextStepTime = audioContext.currentTime + stepDuration;
    setTimeout(() => {
        playStep();
        scheduleNextStep();
    }, nextStepTime * 1000 - audioContext.currentTime * 1000); // Convert to milliseconds
}

// Starting, pausing, and resuming the scheduler functions remain mostly unchanged,
// but they now rely on the pre-calculated schedule and the optimized playStep logic
...
function stopScheduler() {
    ...
    clearTimeout(timeoutId); // Stop any further steps from being scheduled.

    // Reset the sequencer's state variables
    currentStepIndex = 0;
    currentSequenceIndex = 0; // Reset sequence index if you want to start from the first sequence next time
    isPaused = true; // Mark the sequencer as paused or stopped.

    // Reset UI elements related to playhead rendering
    resetPlayheadUI();

    // Additional UI and state resets can be added here
}

// A new function to reset the UI elements for the playhead
function resetPlayheadUI() {
    // Example: Reset all step buttons to their non-playing state
...
function incrementStepCounters() {
    // Increment current step, looping back to 0 after reaching 63
    currentStep = (currentStep + 1) % 64;
    // Increment total step count
    totalStepCount += 1;
    // Increment next step time by step duration
    nextStepTime += stepDuration;

    // Check if current step is a multiple of 4
    if (currentStep % 4 === 0) {
    ...
    // Check if current step is a multiple of 16
    if (currentStep % 16 === 0) {
        // Increment bar count
        barCount += 1;
        // Emit bar event
    ...
    // Check if current step is a multiple of 64
    if (currentStep % 64 === 0) {
...
function handleSequenceTransition(sequence) {
    // Log sequence transition handling
    // console.log("[SeqDebug][stepHandling] handleSequenceTransition entered");
    // Log sequence transition with current sequence
    // console.log(`[SeqDebug] handleSequenceTransition called with sequence: ${sequence}`);
    ...
    // console.log(`[SeqDebug][stepHandling] Sequence set to ${sequence} at ${(new Date).toLocaleTimeString()}`);
